...
Double_t fp(Double_t* xx, Double_t* par) {
   // p(t)
   ...
   if (x < 0) return 0;          // pulse function range = (0, inf)
   if (tau2 < eps) return 0;     // tau2 must be finite
   if (tau1 < 0) return 0;       // sanity check

   Double_t arg;

   Double_t exp_tau1 = 0;
   arg = -x/tau1;
   if (TMath::Abs(arg) < explim) exp_tau1 = TMath::Exp(arg);
   Double_t exp_tau2 = 0;
   arg = -x/tau2;
   if (TMath::Abs(arg) < explim) exp_tau2 = TMath::Exp(arg);

   Double_t norm = (tau1+tau2)/tau2/tau2;
   Double_t fp = A*norm*(1.-exp_tau1)*exp_tau2;
   return fp;
}

void plot_p(
   // Double_t xmin = 49.5, Double_t xmax = 55.0, Double_t x0 = 50
   // Double_t xmin = -0.5, Double_t xmax = 5.0, Double_t x0 = 0
   Double_t xmin = -0.5, Double_t xmax = 10.0, Double_t x0 = 0
   , Double_t A = 1
   , Double_t tau1 = 0.050    // ns
   // , Double_t tau2 = 1.0      // ns
   , Double_t tau2 = 5.0      // ns
   )
{
   Double_t par[10];
   Int_t npar = 0;
   ...
   TF1* fun;
   TF1* fun_p = fun = new TF1("fun_fp", fp, xmin, xmax, npar);
   // fun->SetTitle("fun_fp;time, ns;pulse height, mV");
   // fun->SetTitle(";ns;mV");
   // fun->SetTitle("");
   fun->SetNpx(1024);
   fun->SetLineColor(1);
   ...
   TPaveText* pave = new TPaveText(0.60,0.50, 0.80,0.74, "NDC");
   pave->SetFillColor(0);
   pave->SetTextAlign(12);
   pave->AddText(Form("#tau_{1} = %0.3f",tau1));
   pave->AddText(Form("#tau_{2} = %0.1f",tau2));
   pave->Draw();
}
...
Double_t fP(Double_t *xx, Double_t *par)
{
   ...
   if (x < 0) return 0;                            // function range (0, inf)
   if (tau1 < 0 || tau2 < 0 || T < 0) return 0;    // sanity check
   
   Double_t norm = A*(tau1+tau2)/tau2/tau2;
   Double_t tau12 = tau1*tau2/(tau1+tau2);

   Double_t fP = norm * (ITtau(x,tau2,T) - ITtau(x,tau12,T));
   return fP;
}

void plot_P(
   Double_t xmin = -0.5, Double_t xmax = 100.0, Double_t x0 = 0
   , Double_t A = 1
   , Double_t tau1 = 0.050    // ns
   // , Double_t tau2 = 1.0      // ns
   , Double_t tau2 = 5.0      // ns
   ...
   )
{
   ...
   TF1* fun;
   TF1* fun_P = fun = new TF1("fun_P", fP, xmin, xmax, npar);
   // fun->SetTitle("fun_fp;time, ns;pulse height, mV");
   // fun->SetTitle(";ns;mV");
   // fun->SetTitle("");
   fun->SetNpx(1024);
   ...
   TPaveText* pave = new TPaveText(0.60,0.50, 0.85,0.74, "NDC");
   pave->SetFillColor(0);
   pave->SetTextAlign(12);
   pave->AddText(Form("#tau_{1} = %0.3f",tau1));
   pave->AddText(Form("#tau_{2} = %0.1f",tau2));
   pave->AddText(Form("T = %0.1f",T));
   pave->Draw();

   cout<< "ftemp()->Integral(xmin,xmax) " << ftemp()->Integral(xmin,xmax) <<endl;
   cout<< "ftemp()->GetX(ftemp()->GetMaximum()/2.72, ftemp()->GetMaximumX(),80) = " << ftemp()->GetX(ftemp()->GetMaximum()/2.72, ftemp()->GetMaximumX(),80) <<endl;
}

//------------------- smearing with resolution function ----------------------------
...
void plot_Psigma(
   Double_t xmin = -10., Double_t xmax = 100.0, Double_t x0 = 0
   , Double_t A = 1
   , Double_t tau1 = 0.050    // ns
   // , Double_t tau2 = 1.0      // ns
   , Double_t tau2 = 5.0      // ns
   ...
   )
{
   ...
   TF1* f;
   TF1* fun_Psigma = f = new TF1("fun_Psigma", fPsigma, xmin, xmax, npar);
   // f->SetTitle("fun_fp;time, ns;pulse height, mV");
   // f->SetTitle(";ns;mV");
   // f->SetTitle("");
   f->SetNpx(1024);
   f->SetLineColor(1);
   ...
   TPaveText* pave = new TPaveText(0.60,0.50, 0.85,0.80, "NDC");
   pave->SetFillColor(0);
   pave->SetTextAlign(12);
   pave->AddText(Form("#tau_{1} = %0.3f",tau1));
   pave->AddText(Form("#tau_{2} = %0.1f",tau2));
   pave->AddText(Form("T = %0.1f",T));
   pave->AddText(Form("#sigma = %0.1f",sigma));
   pave->Draw();
...
//---------------------- compare with std version of pulsetools.C --------------------------

// std version of ITtausigma from pulsetools.C
Double_t ITtausigma_std(Double_t x, Double_t tau, Double_t T, Double_t sigma)
{
   using TMath::Pi;
   using TMath::Exp;
   using TMath::Sqrt;
   ...
   if (sigma < eps)
   {
      ...
      Double_t erfc_T = 0;
      arg = (x-sigma*sigma/T)/sigma/Sqrt(2.);
      if (TMath::Abs(arg) < erfclim) erfc_T = TMath::Erfc(arg);
      else if (arg < 0) erfc_T = 2;
      else erfc_T = 0;
      Double_t ITsigma = 0.5 * exp_xT * exp_sigma2T2 * erfc_T;
      ...
      Double_t erfc_tau = 0;
      arg = (x-sigma*sigma/tau)/sigma/Sqrt(2.);
      if (TMath::Abs(arg) < erfclim) erfc_tau = TMath::Erfc(arg);
      else if (arg < 0) erfc_tau = 2;
      else erfc_tau = 0;
      Double_t Itausigma = 0.5 * exp_xtau * exp_sigma2tau2 * erfc_tau;

      //cout<< "ITsigma = " << ITsigma << " Itausigma = " << Itausigma <<endl;
...
// std version of Psigma from pulsetools.C
Double_t Psigma_std(Double_t x, Double_t tau1, Double_t tau2, Double_t T, Double_t sigma)
{
   ...
   Double_t tau12 = tau1*tau2/(tau1+tau2);
   Double_t ITtausigma2 = ITtausigma_std(x,tau2,T,sigma);
   Double_t ITtausigma12 = ITtausigma_std(x,tau12,T,sigma);
   Double_t norm = (tau1+tau2)/tau2/tau2;
   Double_t Psigma = norm * (ITtausigma2 - ITtausigma12);
   return Psigma;
}

// std version of fPsigma from pulsetools.C
Double_t fPsigma_std(Double_t* xx, Double_t* par)
{
   const Double_t eps = 1.e-12;
   ...
   return A*Psigma_std(x,tau1,tau2,T,sigma);
}

Double_t fPsigma_compare(Double_t* xx, Double_t* par)
{
   return fPsigma(xx,par) - fPsigma_std(xx,par);
}

void plot_Psigma_compare(
   Double_t xmin = -10., Double_t xmax = 100.0, Double_t x0 = 0
   , Double_t A = 1
   , Double_t tau1 = 0.050    // ns
   // , Double_t tau2 = 1.0      // ns
   ...
   )
{
   ...
   // std (old) code
   TF1* fun_Psigma_std = f = new TF1("fun_Psigma_std", fPsigma_std, xmin, xmax, npar);
   f->SetNpx(10240);
   f->SetLineColor(1);
   f = 0;

   // compare new code with std
   TF1* fun_Psigma_compare = f = new TF1("fun_Psigma_compare", fPsigma_compare, xmin, xmax, npar);
   f->SetNpx(10240);
   f->SetLineColor(1);
   f = 0;

   fun_Psigma->SetParameters(par);
   fun_Psigma_std->SetParameters(par);
   fun_Psigma_compare->SetParameters(par);
      ...
      )
{
   ...
   cout<< "fit_x0 = " << fit_x0 << " fit_tau2 = " << fit_tau2 <<endl;
