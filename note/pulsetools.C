   ...
   for (int jentry=entry_first; jentry<=entry_last; ++jentry)
   {
      ...
      for (int ich=0; ich<nchannels; ++ich)
      {
         ...
         for (int i=0; i<1024; ++i)
         {
            ...
            //if (ifirst == 0 && x[i] > sigmin) ifirst = i;
            //if (x[i] > sigmin && x[i] < sigmax) ++np;
            //if (x[i] > sigmin && x[i] < sigmax) {
            if (x[i] > sigmin) {
            ...
            // maximum in signal window
            if (x[i] > sigmin && x[i] < sigmax) if (y[i] > thres) ++nthres;
            if (x[i] > sigmin && x[i] < sigmax) if (y[i] > ampl_max) ampl_max = y[i];
         }
         // cout<< "-- np = " << np <<endl;
         ...
         grsig = new TGraphErrors(np, &x[ifirst], &y[ifirst], &ex[ifirst], &ey[ifirst]);
         ...
         Double_t ymax = 0;      // initialyze by 0
         for (int i=0; i<np; ++i) {
            // FIXME: use (temporary, should be fixed) signal region
            if (x[i] < sigmin) continue;
            if (x[i] > sigmax) break;
         ...
         Int_t iedge_first = iedge_min;                  // consider iedge_min as a first edge data point
         Int_t iedge_last = iedge_max+1;                 // fit upper limit
         Int_t np_edge = iedge_last - iedge_first + 1;   // the number of points on the edge
         oscFit->np[ich] = np_edge;                      // store in the tree
         // fit to the next point because likely the iedge_max is higher for DRS4
         pulse_x2 = x[iedge_max+1] + epsline;
         ...
         x0 = 60.;         // clipping C
         // tau1 = 0.050;
         tau1 = 0.200;        // clipping C
         // tau2 = 2.0;    // clipping C
         tau2 = 10.0;
         // T = 20;        // clipping C
         T = 40;
         sigma = 0.200;       // clipping C
         ...
         if (npoint_fit > 2)
         {
            ...
            //cout<< "--> got fitted function to calc integral" <<endl;
         ...
         oscFit->adcf[ich] = integralf;
         // oscFit->t[ich] = y0cross;
         // oscFit->T[ich] = y05cross;

         if (fitted_function)
         {
            //. take T and tau2 from the first fit
            //oscFit->T[ich] = fitted_function->GetParameter("T");
            //oscFit->tau2[ich] = fitted_function->GetParameter("tau2");
            ...
            if (fitted_function_refit)
            {
               ...
               // Double_t middle_point_x = pulse_xhalfmax;
               //---------------------Double_t middle_point_x = (fit_edge_xmin + fit_edge_xmax) / 2.;
               // Double_t middle_point_x = (x[iedge_min] + x[iedge_max]) / 2.;
